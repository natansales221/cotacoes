...
class ExtractIbovespa:
    TICKER = "^BVSP"
    TIPO = "IBOVESPA"
    DATA_INICIAL = "2000-01-01"

    def output_path(self):
        base_data = Path(f"data/downloads/yfinance/ibovespa/ibovespa.csv")
        return base_data

    def extract(self):
        # O parametro end do yfinance e exclusivo; por isso usamos amanha.
        ...
        resultado = pd.DataFrame(
            {
                ...
                "fechamento_ajustado": pd.to_numeric(dados.get("Adj Close", dados.get("Close")), errors="coerce"),
